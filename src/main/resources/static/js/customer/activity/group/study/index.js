window.addEventListener("load", (e) => {
	...
	infoBtn.addEventListener("click", (e) => {
		...
		win = open("/customer/activity/group/study/" + id + "/info", "_blank", "width=1080px,height=650px");
...
window.addEventListener("load", (e) => {
	...
	let studyId = document.querySelector("input").value;
	let boardId = 0;
	let boardCnt = 0;
	let currentCnt = 0;
	...
	function getBoardList() {
		fetch(`/api/studyBoardController/list?studyId=${studyId}`)
	...
	boardRegBtn.addEventListener("click", (e) => {
		boardRegWin = open("/customer/activity/group/study/" + studyId + "/board/reg", "_blank", "width=500px,height=500px");
	});

	function getCommentList(detail, boardId) {
		fetch(`/api/studyCommentController/list?boardId=${boardId}`)
			.then(response => response.json())
			.then(json => {
				let commentList = "";
				let reged = false;
	...
	function createNode(detail, boardId) {
		...
		cmtRegBtn.addEventListener("click", (e) => {
			...
			fetch(`/api/studyCommentController/reg`,
				{
					method: "POST",
					headers: {
				...
				.then(() => {
					fetch(`/api/studyCommentController/cnt?boardId=${boardId}`)
						.then(response => response.json())
						.then(cnt => {
	...
	function createDetailDynamicNode(authId, boardWriterId, ...nodes) {
		...
		detailEdit.addEventListener("click", (e) => {
			...
			let win = open("/customer/activity/group/study/" + studyId + "/board/edit?id=" + boardId, "_blank", "width=500px,height=500px");
		...
		detailDel.addEventListener("click", (e) => {
			...
			if (!result)
				return;
			let id = boardId;
			fetch(`/api/studyBoardController/delete?id=${id}`)
				.then(() => {
